# This file runs an experiment for the continuous measurement of a qubit through homodyne detection.
...
function Eff_QFI_HD(
    ...
    result = @distributed (+) for ktraj = 1 : Ntraj
        ...
        for jt = 1 : Ntime
            ...
            dy = dt * sqrt(η) * tr(ρ * (c + c')) + dW()     # Homodyne current 

            # Kraus operator with Euler-Milstein correction
            M = M0 + sqrt(η) * c * dy + η/2 * c * c * (dy^2 - dt)

            # Evolve the density operator
            new_ρ = M * ρ * M' + (1 - η) * dt * c * ρ * c' 
            ...
            τx = (M * (τx * M' + ρ * dxM') + dxM * ρ * M' + (1 - η)* dt * c * τx * c' )/ tr_ρ
            τy = (M * (τy * M' + ρ * dyM') + dyM * ρ * M' + (1 - η)* dt * c * τy * c' )/ tr_ρ
